...
while True:
    try:
        ...
        if 1000 <= year <= 9999:
            ...
            slowtype("That isn't quite true though, is it? Let's give it another go: ")
    except ValueError:
        slowtype("You need to enter an actual number. Or are you too stupid to grasp that?: ")
# this makes sure only valid inputs are given
  
if year in range(1950, 2000):
    slowtype("%s \n" % "Damn. You are old.")
elif year == 2001:
    slowtype("%s \n" % "A good year!")
    ...
    sys.exit()
elif year in range(1901, 1949):
    slowtype("%s \n" % "How the hell do you know how to use a computer?")
elif year <= 1900:
    slowtype("%s \n" % "Did you dig yourself out of your own grave? Go away, zombie!")
    sys.exit()


# this allows for different outputs based on the year, and will exit the program entirely if they're deemed "too young"
# fixed this so that it will only accept a viable year
# (even with the conditions that spit out different statements, it continued for other inputs like "90" or "1")
time.sleep(0.85)
...
while month in monthdatematrix:
    try:
        ...
        slowtype("You need to enter an actual number, idiot: ")
...
slowtype("\nWhat aspect of your future would you like to find out about?") 
time.sleep(0.2)
...
while True:
    ...
    if horoscope not in future:
        slowtype("%s \n" % "Is that what I asked you to do? No. You will have to start again.")
    ...
    while True:
        try:
            ...
            slowtype("You were supposed to type \"yes\" or \"no\", you incompetent fool.")
            sys.exit()
        # so it only continues for yes/no inputs
...
def exitsequence():
    slowtype("\nThank you! I have been the birthday chatbot, hope you have an average day.")
    sys.exit()
