...
while True:
    ...
    slowtype("\nDo you want to learn about another aspect of your future? (yes/no) ")
    if input().lower() == "no":
        break
    else:
        slowtype("\nChoose from: LOVE, HEALTH, FAMILY, WORK, FINANCE, and TRAVEL \nInput your choice here: ")
...
slowtype("\nWould you like to know the celebrities you share a birthday with? (yes/no) ")

if input().lower()!= "no":  #if input is not 'no'
    slowtype("\nHere we go!")
else:
    slowtype("Goodbye!")
    time.sleep(0.85)
