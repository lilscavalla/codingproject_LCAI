...
## horoscope options (to randomise)
...
#### DO YOU WANT YOUR HOROSCOPE OR CELEBRRITY BIRTHDAY???
#### LOOP THE 2 BRANCHES AS INDIVIDUAL "SETS", OR AS SEPERATE CODE FILES TO RUN?
#### thing to trasnlate bday dates so you don't have to input it twice
...
for item in inputmonth:
    for item in inputdate:
        ...
        # this can be put as one loop if you use AND between clasifiers? but it'll have to be bracketed?
...
while True:
    ...
    if horoscope == "love" and ss == horoscopesstarsign[0]:
        selected_horoscope = random.choice(lovelist0)
        slowtype("Your love horoscope is: {}\n".format(selected_horoscope))
        break
    else:
        slowtype("Invalid horoscope or star sign. Please try again.\n")

# ^^^ this is the only one that works, i need to reformat the conditions below to match it
...
# we want it to give the option to get another answer for a different choice, and LOOP back to do it again
# OR if they dont want another, to exit the loop and go to the next thing


    while True:
        morefuture = input().lower()
        slowtype("\nDo you want to learn about another aspect of your future? Please type yes/no: ")
        if morefuture != "no":
            slowtype("\nChoose from: LOVE, HEALTH, FAMILY, WORK, FINANCE, and TRAVEL \nInput your choice here: ")
        else:
            break
        ...
        slowtype("%s \n" % "Did I say I could do that? Try again: ")
